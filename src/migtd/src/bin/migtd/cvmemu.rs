...
fn initialize_emulation() {
    ...
    let result = {
        ...
        // Initialize with policy v2 (no separate root CA needed)
        let chain_path_static: &'static str =
            Box::leak(policy_issuer_chain_file_path.into_boxed_str());
        td_shim_interface_emu::init_file_based_emulation_policy_v2(policy_path, chain_path_static)
    };
...
fn parse_commandline_args() {
    ...
    let mut operation: &str = "migration"; // "migration", "rebind-prepare", "rebind-finalize"
