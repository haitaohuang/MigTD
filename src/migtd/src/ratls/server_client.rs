...
mod verify {
    ...
    fn verify_public_key(verified_report: &[u8], public_key: &[u8]) -> CryptoResult<()> {
        ...
        if cfg!(feature = "no-get-quote") {
            // In no-get-quote mode, mock quote is used for testing.
            // Bypass public key hash check in this development environment.
            log::warn!(
                "no-get-quote mode: Skipping public key verification in TD report. This is NOT secure for production use.\n"
            );
            return Ok(());
        }
