...
pub extern "C" fn servtd_get_quote(tdquote_req_buf: *mut c_void, len: u64) -> i32 {
    ...
    match tdvmcall_get_quote(shared.as_mut_bytes()) {
        ...
        Err(TdVmcallError::VmcallRetry) => {
            ...
            return AttestLibError::Again as i32;
        }
        Err(e) => {
...
fn wait_for_quote_completion(notify_registered: bool, buffer: &[u8]) -> Result<(), AttestLibError> {
    ...
    match status_code {
        ...
        GET_QUOTE_STATUS_IN_FLIGHT => {
            log::error!(
                "Quote status indicates retriable error: {:#x}\n",
                status_code
            );
            Err(AttestLibError::Again)
        }
        GET_QUOTE_STATUS_SERVICE_UNAVAILABLE | GET_QUOTE_STATUS_ERROR => {
            log::error!(
