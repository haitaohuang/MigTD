...
use crate::provider::{azure_thim::AzureThimProvider, intel_pcs::IntelPcsProvider};
...
pub fn merge_collaterals(
    collaterals_list: Vec<Collaterals>,
    output_collateral: &PathBuf,
) -> Result<Collaterals> {
...
/// Write collaterals to a JSON file with pretty printing
fn write_collaterals_file(collateral_output: &PathBuf, collaterals: &Collaterals) -> Result<()> {
    ...
    let json_data = serde_json::to_vec_pretty(collaterals)?;
