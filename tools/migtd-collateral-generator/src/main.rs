...
fn main() {
    ...
    let provider_type = match config.provider.to_lowercase().as_str() {
        ...
        "azure-thim" | "azure" | "thim" => {
            ...
            let regions: Vec<String> = if config.azure_region == "all-us-europe" {
                vec!["useast".to_string(), "westus".to_string(), "northeurope".to_string()]
            } else {
                config.azure_region.split(',').map(|s| s.trim().to_string()).collect()
            };

            if regions.len() > 1 {
                // Fetch from multiple regions and merge
                println!("Fetching collaterals from {} regions: {:?}", regions.len(), regions);
                
                let mut all_collaterals = Vec::new();
                for region in &regions {
                    println!("\nFetching from {}...", region);
                    // Create temporary output path for this region
                    let temp_output = config.output.with_file_name(
                        format!("{}_temp_{}.json", 
                ...
                println!("\nMerging collaterals from {} region(s)...", all_collaterals.len());
                match merge_collaterals(all_collaterals, &config.output) {
                    Ok(merged) => {
                        println!("✓ Merged successfully: {} unique platform(s)", merged.platforms.len());
        ...
        _ => {
            eprintln!("Error: Invalid provider '{}'. Valid options: 'intel', 'azure-thim'", config.provider);
            exit(1);
        }
    };
