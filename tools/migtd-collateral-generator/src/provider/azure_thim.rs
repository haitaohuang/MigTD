...
const INTEL_PCS_FMSPC_LIST_URL: &str = "https://api.trustedservices.intel.com/sgx/certification/v4/fmspcs";
...
impl AzureThimProvider {
    ...
    fn fetch_fmspc_list_from_intel(&self) -> Result<Vec<String>> {
        ...
        match pcs_response.response_code {
            ...
            _ => {
                eprintln!("Error fetching FMSPC list from Intel - {:?}", pcs_response.response_code);
...
impl CollateralServiceProvider for AzureThimProvider {
    fn fetch_pck_crl(&self) -> Result<(Vec<u8>, String)> {
        ...
        match pcs_response.response_code {
            ...
            _ => {
                eprintln!("Error fetching PCK CRL from Azure THIM - {:?}", pcs_response.response_code);
    ...
    fn get_platform_tcb_list(&self) -> Result<Vec<PlatformTcbRaw>> {
        ...
        println!("Attempting to fetch TCB info for {} FMSPCs from Azure THIM ({})...", fmspc_list.len(), self.region);
        ...
        if platform_tcb_list.is_empty() {
            println!("Warning: No TCB info available from Azure THIM in {} region.", self.region);
            println!("This is expected if the region doesn't have TDX platforms deployed yet.");
        } else {
            println!("Successfully fetched TCB info for {} platform(s)", platform_tcb_list.len());
        }
    ...
    fn fetch_platform_tcb(&self, fmspc: &str) -> Result<Option<(Vec<u8>, String)>> {
        ...
        let result = if pcs_response.response_code == 200 {
            ...
            println!("TCB info not found for fmspc {} (404 - may not be deployed in this region)", fmspc);
            None
        } else {
            eprintln!(
