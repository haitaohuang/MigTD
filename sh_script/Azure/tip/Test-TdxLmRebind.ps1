...
param(
    ...
    [string]$OldHashEvidencePath,
    [string]$NewHashEvidencePath,
...
function Start-MigTdInstance {
    ...
    $creation = New-TipHcsMigTd `
        -Id $InstanceId `
        -IgvmFilePath $IgvmPath `
        -GuestStateDirectory $guestStateDirectory `
        -CandidateVmgsPath $CandidateVmgsPath `
        -CandidateEvidencePath (Join-Path (Get-Location) "$InstanceId.vmgs.evidence.json") `
        -EnableSerial:$CaptureSerial `
        -Force
    $script:migTdCreations[$InstanceId] = $creation
    if ($creation.CandidateVmgsEvidencePath) {
        $script:evidenceFiles.Add($creation.CandidateVmgsEvidencePath)
    }
    ...
    try {
        ...
        $startError = $_
        $startCleanupFailures = [System.Collections.Generic.List[string]]::new()
        try {
            Stop-MigTdInstance $instance
        } catch {
            $startCleanupFailures.Add($_.Exception.Message)
        }
        try {
            Remove-TipCandidateVmgsCopy -MigTdCreation $creation
            $script:migTdCreations.Remove($InstanceId)
        } catch {
            $startCleanupFailures.Add("Candidate VMGS cleanup for $InstanceId failed: $($_.Exception.Message)")
        }
        if ($startCleanupFailures.Count -gt 0) {
            throw (New-TipHarnessException 'CLEANUP' "$($startCleanupFailures -join '; ') | PrimaryError=$($startError.Exception.Message)")
        }
        throw $startError
    }
}
...
try {
    ...
    foreach ($instanceId in @($NewMigTdId, $OldMigTdId)) {
        $creation = $script:migTdCreations[$instanceId]
        if ($creation) {
            try {
                Remove-TipCandidateVmgsCopy -MigTdCreation $creation
                $script:migTdCreations.Remove($instanceId)
            } catch {
                $cleanupFailures.Add("Candidate VMGS cleanup for $instanceId failed: $($_.Exception.Message)")
            }
        }
    }
}
