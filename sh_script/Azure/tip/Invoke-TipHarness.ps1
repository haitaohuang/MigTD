...
function Get-EvidenceList {
    ...
    $evidenceProperty = $InvocationResult.PSObject.Properties['EvidenceFiles']
    if ($evidenceProperty -and $evidenceProperty.Value) {
        foreach ($entry in @($evidenceProperty.Value)) {
            if ($entry) {
                $evidenceFiles.Add([string]$entry)
            }
        }
    }
...
function Invoke-TipCase {
    ...
    if (Test-Path -LiteralPath $caseLogDir) {
        Remove-Item -LiteralPath $caseLogDir -Recurse -Force
    }
    ...
    try {
        ...
        $evidenceFiles = @($evidence.EvidenceFiles | ForEach-Object {
            if ([System.IO.Path]::IsPathRooted($_)) { $_ } else { Join-Path $caseLogDir $_ }
        })
        ...
        if ($RunCaptureSerial) {
            ...
            foreach ($path in $evidenceFiles) {
                if (-not (Test-Path $path)) {
                    $evidenceFailures.Add("Case evidence file missing: $path")
                }
            }
    ...
    $serialLogs = @(
        @($serialLogs) +
        @(Get-ChildItem -LiteralPath $caseLogDir -Filter '*.serial.log' -File -ErrorAction SilentlyContinue |
            ForEach-Object { $_.FullName })
    ) | Select-Object -Unique
    $evidenceFiles = @(
        @($evidenceFiles) +
        @(Get-ChildItem -LiteralPath $caseLogDir -Filter '*.evidence.json' -File -ErrorAction SilentlyContinue |
            ForEach-Object { $_.FullName })
    ) | Select-Object -Unique
...
function Get-RunCases {
    ...
    if (-not $IgvmPath) {
        throw (New-TipHarnessException 'PRECONDITION' '-IgvmPath is required for the ReleaseDeep suite.')
    }
    if (-not $CandidateVmgsPath) {
        throw (New-TipHarnessException 'PRECONDITION' '-CandidateVmgsPath is required for the ReleaseDeep suite.')
    }

    $releaseIgvmPath = Get-RunArtifactPath -Path $IgvmPath -PackageRoot $PackageRoot
    $releaseHashFilePath = if ($HashFilePath) {
        Get-RunArtifactPath -Path $HashFilePath -PackageRoot $PackageRoot
    } else {
        "$releaseIgvmPath.hash"
    }
    $releaseHashEvidencePath = if ($HashEvidencePath) {
        Get-RunArtifactPath -Path $HashEvidencePath -PackageRoot $PackageRoot
    } else {
        Get-TipHashEvidencePath -HashFilePath $releaseHashFilePath
    }
    $releaseVmgsPath = Get-RunArtifactPath -Path $CandidateVmgsPath -PackageRoot $PackageRoot
    $releaseCliPath = if ($SvmApiCliPath) {
        Get-RunArtifactPath -Path $SvmApiCliPath -PackageRoot $PackageRoot
    } else {
        $null
    }
    ...
    return @(
        @{
            ...
            Script = 'Invoke-TdxLmLoopback.ps1'
            ExpectedOutcome = 'Success'
            Args = @{
                IgvmFilePath = $releaseIgvmPath
                HashFilePath = $releaseHashFilePath
                HashEvidencePath = $releaseHashEvidencePath
                CandidateVmgsPath = $releaseVmgsPath
                MigTdId = 'tipmigtd-rel-loopback'
                VmName = 'tiptd-rel-loopback'
                NoPersistentSecrets = $false
            }
        },
        @{
            Name = 'release-rebind-same-image'
            Script = 'Test-TdxLmRebind.ps1'
            ExpectedOutcome = 'Success'
            Args = @{
                OldIgvmFilePath = $releaseIgvmPath
                NewIgvmFilePath = $releaseIgvmPath
                OldHashFilePath = $releaseHashFilePath
                NewHashFilePath = $releaseHashFilePath
                OldHashEvidencePath = $releaseHashEvidencePath
                NewHashEvidencePath = $releaseHashEvidencePath
                CandidateVmgsPath = $releaseVmgsPath
                OldMigTdId = 'tipmigtd-rel-rebind-old'
                NewMigTdId = 'tipmigtd-rel-rebind-new'
                VmName = 'tiptd-rel-rebind'
            }
        },
        @{
            Name = 'release-gettdreport'
            Script = 'Test-TdxGetTdReport.ps1'
            ExpectedOutcome = 'Success'
            Args = @{
                IgvmFilePath = $releaseIgvmPath
                HashFilePath = $releaseHashFilePath
                HashEvidencePath = $releaseHashEvidencePath
                CandidateVmgsPath = $releaseVmgsPath
                SvmApiCliPath = $releaseCliPath
                MigTdId = 'tipmigtd-rel-gettdreport'
            }
        }
    )
}
...
function Invoke-RunMode {
    ...
    try {
        ...
        $cases = Get-RunCases -SelectedSuite $Suite -PackageRoot $runPackageDir
        foreach ($case in $cases) {
            ...
            if ($caseResult.category -eq 'CLEANUP') {
                Write-Warning "Aborting suite after fail-closed cleanup failure in '$($caseResult.name)'."
                break
            }
        }
    } catch {
    ...
    $summary = [ordered]@{
        ...
        abortedAfterCleanupFailure = @($failedCases | Where-Object { $_.category -eq 'CLEANUP' }).Count -gt 0
