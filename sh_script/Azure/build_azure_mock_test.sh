...
#   # migtd image built with no-get-quote feature, which uses the same mock data for
#   # quote/report verification.
#   ./sh_script/Azure/build_azure_mock_test.sh --mock-report
#
...
if [ "$USE_MOCK_REPORT" = true ]; then
    echo -e "${YELLOW}  Note: Mock data will be tested ${NC}"
fi
echo
