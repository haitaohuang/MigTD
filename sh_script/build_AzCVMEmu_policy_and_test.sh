...
#
#   # Fetch fresh collaterals from Azure THIM before generating policy (single region)
#   ./sh_script/build_AzCVMEmu_policy_and_test.sh --fetch-collaterals --azure-region useast
#
#   # Fetch collaterals from all US and Europe regions combined
#   ./sh_script/build_AzCVMEmu_policy_and_test.sh --fetch-collaterals --azure-region all-us-europe
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --azure-region REGION        Azure region for THIM (useast, westus, northeurope, or all-us-europe)"
            echo "                               Use 'all-us-europe' to fetch and combine from all US and Europe regions"
            ...
            echo "  $0 --mock-report"
            echo
            echo "  # Fetch fresh collaterals from Azure THIM (single region) and generate policy:"
            echo "  $0 --fetch-collaterals --azure-region useast"
            echo
            echo "  # Fetch collaterals from all US and Europe regions combined:"
            echo "  $0 --fetch-collaterals --azure-region all-us-europe"
            echo
...
if [ "$FETCH_COLLATERALS" = true ]; then
    ...
    if [ -n "$FMSPC_LIST" ]; then
        echo "  Custom FMSPCs: $FMSPC_LIST"
    fi
fi
echo
...
if [ "$FETCH_COLLATERALS" = true ]; then
    ...
    # Fetch collaterals (tool handles multi-region merging automatically)
    # All temporary files will be created in $TEMP_DIR and cleaned up automatically
...
#
# Always build the test command consistently
