...
CORIM_CLI_INSTALL_ROOT="${CORIM_CLI_INSTALL_ROOT:-$PROJECT_ROOT/target/corim-cli-$CORIM_CLI_REV}"
...
# Only one CoRIM maps the shared mock hash. The integration test attaches the
# pair in both orientations and requires the peer with the missing mapping to
# be rejected.
generate_signed_corim \
    "$MOCK_TDINFO_HASH" \
    2 \
...
jq '(.policyData.policy[] | select(has("servtd"))
        .servtd.migtdIdentity.isvsvn) = {
