...
pub fn init_file_based_emulation_with_policy_chain(
    ...
    // First call the base implementation for policy and root CA
    let base_loaded = init_file_based_emulation_with_real_files(policy_path, root_ca_path);

    // Then load the policy issuer chain
    let chain_loaded = crate::td_uefi_pi::fv::load_policy_issuer_chain_from_file(policy_issuer_chain_path);

    base_loaded && chain_loaded
}
