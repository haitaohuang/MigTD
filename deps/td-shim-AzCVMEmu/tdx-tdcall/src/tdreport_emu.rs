...
fn create_td_report_from_file(quote_file_path: String) -> tdx::TdReport {
    ...
    let (body_offset, body_size, servtd_hash_opt) = if header_version == 4 {
        ...
        (QUOTE_HEADER_SIZE, QUOTE_V4_BODY_SIZE, None)
        ...
        (QUOTE_HEADER_SIZE + 6, body_size, if body_type == 3 { Some([0u8; 48]) } else { None })
    } else {
    ...
    let (report_body, servtd_hash) = if body_size == QUOTE_V5_BODY_SIZE_15 {
        ...
        (base_body, [0x4E; 48]) // Mock value for v4/v5.0 (not in quote body)
    };
    ...
    let td_report = TdxReport {
        ...
        td_info: TdInfo {
            ...
            servtd_hash: servtd_hash, // From v5.5 quote or mock value for v4/v5.0
            reserved: [0u8; 64],
        },
    };
