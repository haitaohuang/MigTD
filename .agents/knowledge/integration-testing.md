...
- Build package:
  ...
  signer anchor (same root + enrolled signer-purpose EKU) but rotate
  the CFV signer leaf key, allowing bidirectional rebind. Peer compatibility
  is based on the common RTMR1 anchor, not leaf Subject Name or leaf-key
  equality. No image must pre-list future rotation measurements in its local
  mapping.
