...
The user has stated **every test in every workflow must pass**. Before declaring a change done, exercise all four workflows locally or confirm via CI:

| Workflow | What it runs | Local invocation |
|---|---|---|
| `.github/workflows/format.yml` | `cargo fmt --check`, `cargo check`, `cargo clippy --features stack-guard,virtio-vsock,virtio-serial,vmcall-interrupt` (no `-D warnings`) | Run each command directly; clippy step does *not* promote warnings to errors. |
| `.github/workflows/deny.yml` | `cargo deny check advisories`, `cargo deny check sources` (continue-on-error), `cargo deny check bans` | `cargo install cargo-deny` then run each. |
| `.github/workflows/main.yml` | 32 `cargo image` builds = 4 devices × 2 policy_versions × 2 protocols × 2 build_types | See §6 for matrix details. |
| `.github/workflows/integration-emu.yml` | 14 EMU scenarios (skip-ra, policy-v2, policy-v2-igvm, rebind-skip-ra, rebind-mock, spdm-skip-ra, spdm-policy-v2, spdm-rebind-skip-ra, mock-quote-retry, policy-v2-key-rotation, policy-v2-key-rotation-igvm, spdm-policy-v2-key-rotation, policy-v2-policy-mapping-rotation, policy-v2-all-chains-rotation) | See §6 for individual flags. |
